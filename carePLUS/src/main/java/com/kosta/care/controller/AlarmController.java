...
public class AlarmController {
	...
	public ResponseEntity<String> sendAlarm(@PathVariable Long empNum){
		try {
			alarmService.sendAlarmByToken(empNum, "알림확인제목", "알림확인내용");
	...
	public ResponseEntity<Boolean> changeAlarmStatus(@RequestBody Map<String, Object> param){
		Long empNum = (Long)param.get("empNum");
		try {
			return new ResponseEntity<Boolean>(alarmService.changeAlarmStatus(empNum), HttpStatus.OK);
		}catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<Boolean>(HttpStatus.BAD_REQUEST);
		}
	...
	public ResponseEntity<Boolean> checkAlarmStatus(@RequestBody Map<String, Object> param){
		Long empNum = (Long)param.get("empNum");
		try {
			return new ResponseEntity<Boolean>(alarmService.checkAlarmStatus(empNum), HttpStatus.OK);
		}catch (Exception e) {
			e.printStackTrace();
			return new ResponseEntity<Boolean>(HttpStatus.BAD_REQUEST);
		}
